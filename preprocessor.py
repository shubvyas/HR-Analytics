...
def clean_data(df):
    ...
    # Convert 'ReviewDate' to datetime, forcing invalid dates to NaT
    if 'ReviewDate' in df_cleaned.columns:
    # Apply conversion only if the date is not already in 'YYYY-MM-DD' format
    ...
    # Convert to MySQL-compatible date format, handle NaT values explicitly
        df_cleaned['ReviewDate'] = df_cleaned['ReviewDate'].apply(
...
def Uploading_file_in_database(df):
    ...
    def load_data_to_db(engine, df, table_name):
        truncate_table(engine, table_name)  # Truncate table first
